...
/* ============================================================
   11. FOOTER LOCATIONS MAP — Leaflet markers (no API key)
   ============================================================ */
(function initFooterLocationsMap() {
  const el = document.getElementById('footer-locations-map');
  if (!el || typeof L === 'undefined') return;

  // Default fallback (Sri Lanka / Colombo area)
  const fallbackLocations = [
    { name: 'Head Office (Colombo)', lat: 6.9271, lng: 79.8612 },
  ];
  ...
  function addLocationMarker(loc) {
    ...
    const marker = L.marker([loc.lat, loc.lng]).bindPopup(`<strong>${escapeHtml(title)}</strong>`);
    markers.addLayer(marker);
  }

  function fitToMarkersOrFallback() {
    if (markers.getLayers().length > 0) {
      map.fitBounds(markers.getBounds().pad(0.25), { animate: false });
      return;
    }
    map.setView([7.8731, 80.7718], 7, { animate: false });
  }
  ...
  async function tryLoadGeoJSON() {
    // If you export Google My Maps as GeoJSON, save it as `locations.geojson`
    // in the site root and the markers will render automatically.
    try {
      ...
      const layer = L.geoJSON(geojson, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          const title = props.name || props.title || props.description || 'Location';
          layer.bindPopup(`<strong>${escapeHtml(title)}</strong>`);
        },
      });

      layer.eachLayer(l => markers.addLayer(l));
      return markers.getLayers().length > 0;
    } catch {
      return false;
    }
  }
  ...
  (async () => {
    const loaded = await tryLoadGeoJSON();
    if (!loaded) fallbackLocations.forEach(addLocationMarker);
    fitToMarkersOrFallback();
  })();

  // If footer is revealed after layout, Leaflet needs a refresh.
